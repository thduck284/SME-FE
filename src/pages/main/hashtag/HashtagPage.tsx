...
import { getPostsByHashtag } from "@/lib/api/posts/GetPostsByHashtag"
...
export function HashtagPage() {
  ...
  useEffect(() => {
    ...
    const fetchPosts = async () => {
      try {
        setLoading(true)
        const res = await getPostsByHashtag(hashtag, 10)
        ...
        if (postsData.length > 0) {
          ...
          setHasMore(!!res.nextCursor) // Has more if nextCursor exists
        } else {
          setPosts([])
          setHasMore(false)
        }
  ...
  const loadMorePosts = useCallback(async () => {
    if (!nextCursor || loadingMore || !hasMore || !hashtag) {
      return
    }
    ...
    try {
      ...
      const [res] = await Promise.all([
        getPostsByHashtag(hashtag, 10, nextCursor),
  ...
  }, [nextCursor, loadingMore, hasMore, hashtag])
  ...
  if (loading) return <HashtagLoading />
  if (error) return <HashtagError error={error} />
  if (posts.length === 0) return <HashtagEmpty hashtag={hashtag || ''} />
...
function HashtagError({ error }: { error: string }) {
  return (
    <div className="min-h-screen flex">
      {/* Left Sidebar - Fixed */}
      <div className="fixed left-0 top-0 h-screen z-10">
...
function HashtagEmpty({ hashtag }: { hashtag: string }) {
  return (
    <div className="min-h-screen flex">
      {/* Left Sidebar - Fixed */}
      <div className="fixed left-0 top-0 h-screen z-10">
      ...
      <main className="flex-1 bg-gray-50 p-6 ml-64 mr-64">
        <div className="max-w-4xl mx-auto">
          <Card className="p-12 text-center">
            ...
            <p className="text-muted-foreground">No posts found with hashtag #{hashtag}.</p>
          </Card>
        </div>
      </main>
