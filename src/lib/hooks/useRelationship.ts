...
export const useRelationship = (): UseRelationshipReturn => {
  ...
  const fetchFollowers = useCallback(async () => {
    ...
    try {
      const followersData = await relationshipService.getFollowers()
  ...
  const fetchFollowing = useCallback(async () => {
    ...
    try {
      const followingData = await relationshipService.getFollowing()
...
// Hook cho relationship (không cần userId)
export const useUserRelationship = () => {
  const { 
    followers, 
    following, 
    loading, 
